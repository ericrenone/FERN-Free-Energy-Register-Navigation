...
FERN derives two core formal results in this regime — a Complexity Criterion (when model expansion is required rather than parameter updating) and a Democratic Weighting result (why contributions from structurally distant positions in the statistical manifold carry higher epistemic value) — and maps these onto the practice of collective sense-making in complex problem environments with explicit action guidance for practitioners and learning designers.
...
Active inference agents minimize variational free energy F through two channels: perception (updating beliefs to better fit observations) and action (changing observations to fit beliefs). In a collective of agents sharing a generative model, this produces coherent joint inference — agents align their posteriors, share sufficient statistics, reduce collective surprise.
...
The dynamics breaks down for a well-defined class of problems: those in which no model at the current depth is adequate. These are problems where:

1. The observations cannot be explained by any parameterization of the current model structure
2. Parameter updating within the existing model reduces F locally but leaves irreducible residual free energy F* that cannot be eliminated without changing the model structure
3. The problem itself changes in response to each intervention, so that no fixed model converges

These are complex problems in the technical sense. They are not merely large or difficult problems with known solution structures. They are problems where the model structure itself is the constraint.
...
This identification — epistemic distance = Fisher-Rao distance — is the load-bearing claim on which FERN's formal results depend. It is stated here as a framework conjecture rather than a theorem because proving it requires showing that the space of epistemic contributions in a human collective learning process constitutes a statistical manifold in the measure-theoretic sense, which requires additional formal work beyond the scope of this paper.
...
The expansion path has lower total expected free energy if and only if the reduction in F*_col achieved by expanding (from F*_col(h) to F*_col(h+1) < F*_col(h)) integrated over the remaining inference horizon exceeds C_expand. In the limit where F*_col(h+1) ≈ 0 (the problem is adequately modeled at depth h+1) and the inference horizon is sufficiently long, this reduces to F*_col(h) > C_expand(h → h+1). ∎

*Note.* This proof is a sketch under simplifying assumptions. A complete proof requires specifying the path integral formulation of collective free energy across the model expansion, which depends on the topology of the statistical manifold M_G in the neighborhood of the expansion barrier. This is identified as a primary target for formal development.

### 3.2 What This Means

The Complexity Criterion is an operational decision rule. It tells a collective — and a practitioner designing collective learning processes — when to stop optimizing within the current framing and when to pursue structural change to the framing itself.

The criterion has two failure modes in practice:
...
**The practical consequence.** The assumption ridge identified in collective sense-making practice — the point at which the existing framing cannot accommodate new contributions and the framing itself must change — is the free energy barrier between model depth h and model depth h+1. Crossing it requires the collective to sustain elevated uncertainty (elevated F_col) without prematurely resolving it through convergence to the prior model.
...
The rate of collective epistemic entropy production — the rate at which the collective's knowledge state expands into previously unoccupied territory of M_G — is bounded below by:
...
DKL[p_i||p_j] measures how much agent i's generative model would be surprised by agent j's predictions. Agents with structurally non-overlapping generative models — different hierarchical depths, different prior structures, different domains of calibration — have high mutual KL divergence. Their encounter produces high information transfer: each agent's contributions are far from the other's current model, producing large Fisher-Rao displacements of M_col.

This makes precise what "cognitive diversity" means in the context of complex collective learning: not diversity of opinion within a shared model structure, but structural diversity of generative models across the agents of the collective. Two expert economists with different views have low D_FERN — their models are structurally similar, differing in parameter values rather than depth or causal structure. An economist, a community health worker, and a directly affected resident have high D_FERN — their models are calibrated at different hierarchical depths, with different prior structures, representing the problem with different causal architectures.
...
*Note.* This proof holds under the assumptions of FERN-C1 (epistemic contributions constitute parameterized probability distributions on a statistical manifold) and in the second-order approximation. Both conditions require additional justification for the general case of human collective learning processes.
...
A collective Markov blanket exists only when the internal states of the collective (the shared generative model M_col and its sufficient statistics) are jointly accessible to all agents through collective sensory states. As Friston et al. show, each agent in a communicating collective achieves lower free energy, with alignment between agents' free energy minima emerging endogenously from their interactions. This endogenous alignment requires that agents can observe each other's sufficient statistics — the shared blanket states.
...
FERN extends the Bayesian mechanics and active inference literature in one specific direction: the far-from-equilibrium collective inference regime that Ramstead et al. (2023) identify as G-theory and leave as future work. The existing literature primarily characterizes:
